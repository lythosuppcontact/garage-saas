...
export default function QuotesPage() {
  ...
  const [companyDefaultVatRate, setCompanyDefaultVatRate] = useState(8.1);
  ...
  const loadContext = async () => {
    ...
    try {
      ...
      if (companyError) {
        ...
        const nextDefaultVatRate = Number(companyData?.default_vat_rate ?? 8.1);

        setBusinessType(nextBusinessType);
        setCompanyDefaultVatRate(nextDefaultVatRate);
        setVatRate(nextDefaultVatRate);
      }
  ...
  useEffect(() => {
    const firstUnit = quoteUi.defaultUnits[0] || "unité";

    setLines((prev) =>
      prev.map((line) => ({
        ...line,
        unit: quoteUi.defaultUnits.includes(line.unit) ? line.unit : firstUnit,
      }))
    );
  ...
  const handleSaveQuote = async () => {
    ...
    try {
      ...
      if (itemsError) {
        ...
        showToast({
          type: "error",
          title: "Erreur création lignes devis",
        });
        return;
      }
      ...
      setCustomerNotes("");
      setInternalNotes("");
      setVatRate(companyDefaultVatRate);
      setLines([
        {
          label: "",
          quantity: 1,
    ...
    <div className="min-h-screen bg-black p-8 text-white">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="space-y-6 rounded-2xl border border-white/10 bg-white/5 p-6">
          <h2 className="text-xl font-semibold">Créer un devis</h2>
          ...
          >
            ...
            <div className="rounded border border-white/10 bg-black/20 p-3">
              ...
              <div className="mt-2 text-xs text-white/40">
                Taux par défaut entreprise : {companyDefaultVatRate}%
              </div>
            </div>
          </div>
